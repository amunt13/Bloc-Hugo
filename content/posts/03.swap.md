...
Així que el tema és com evitar de posar la contrasenya dues vegades, per a la partició arrel i per a la swap. He de dir que he trobat dues solucions. Fer un xifrat automàtic de la swap i l'altra, eliminar-la.

## Una solució: xifrat automàtic de la swap.

Aquesta solució la vaig utilitzar quan estava instal·lant FreeBSD a un portàtil. Es tracta de dir-li al sistema que xifrii la swap a l'arrencada amb Geli. Per fer-ho fes el següent:
...
D'aquesta manera la swap quedarà xifrada i no necessitaràs introduir cap altra contrasenya. Pots verificar l'estat de la swap amb l'ordre `swapinfo`. Si tot és correcte, la sortida tindrà un sufix `.eli`com per exemple `/dev/ada0p4.eli`.
...
Pel què tinc entés això es pot aconseguir de dues maneres, enviant la swap a un fitxer o creant la zram. No activar cap tipus de swap no crec sigui convenient, a no ser que tinguis una quantitat de ram molt superior a la què normalment uses. Ara parlaré de la zram.

### Activar ZRAM.

És la solució que aporta Fedora per defecte en la instal·lació de la versió workstation, que és la que vaig provar. Suprimeix la swap perquè amplia la ram física existent en base a un procés de compressió. Es pot usar sense swap o també amb ella creant un sistema mixt. És evident que si vols evitar el posar la contrasenya per a la swap, el correcte és eliminar-la.
